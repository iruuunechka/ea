...
/**
 * A Java implementation of CMWC4096 algorithm.
 * <p>
 * This implementation is essentially single-threaded, i.e. no methods are synchronized.
 * For the multi-threaded use of this class, the method {@link #threadLocal()} is provided.
 *
...
public final class FastRandom extends Random implements Serializable {
    ...
    private static final ThreadLocal<FastRandom> THREAD_LOCAL = ThreadLocal.withInitial(FastRandom::new);
    ...
    // The Ziggurat parts are adapted from http://www.seehuhn.de/pages/ziggurat,
    // mainly http://m.seehuhn.de/data/ziggurat/gauss.c
    private static final double ZIGGURAT_R = 3.44428647676;
    private static final double[] ZIGGURAT_Y = {
    ...
    private static final int[] ZIGGURAT_K = {
    ...
    private static final double[] ZIGGURAT_W = {
     ...
     * <p>
     * Creation of two fast randoms sequentially will most probably
     * produce two different generators.
     */
    public FastRandom() {
        super();
    }
     ...
     * <p>
     * Unlike some implementations of {@link Random},
     * a {@link FastRandom} either constructed using a seed {@code S}
     * or initialized to {@code S} later will produce the same random sequence.
     *
     * @param seed the seed.
     */
    ...
    //Copied from java.util.Random in order to stop being synchronized.
    final double nextGaussianSlow() {
    ...
    public final double nextGaussian() {
        while (true) {
            ...
            double y;
            if (i < 127) {
                double y0 = ZIGGURAT_Y[i];
                double y1 = ZIGGURAT_Y[i + 1];
                y = y1 + (y0 - y1) * nextDouble();
            } else {
                x = ZIGGURAT_R - Math.log(1 - nextDouble()) / ZIGGURAT_R;
                y = Math.exp(-ZIGGURAT_R * (x - 0.5 * ZIGGURAT_R)) * nextDouble();
            }
